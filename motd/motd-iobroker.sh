...
if [ -f $(which iobroker) ]; then
  ...
  # get host info
  jscontroller_alive=$(iobroker state getvalue system.host.$HOSTNAME.alive | tail -n 1)
  
  # iobroker-master
  if [ $jscontroller_alive == 'true' ]; then
    jscontroller_alive=$green_color'alive'
  elif [ $jscontroller_alive == 'false' ]; then
    jscontroller_alive=$red_color'alive'
  fi
  
  echo
  echo -e $light_blue_color"ioBroker js-controller: $green_color$iobversion$close_color / $jscontroller_alive"$close_color
